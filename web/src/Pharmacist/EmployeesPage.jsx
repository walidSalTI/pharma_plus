...
export default function EmployeesPage() {
  ...
  const fetchEmployees = () => {
    ...
    employeeService
      ...
      .then((res) => setEmployees(res?.data ?? res?.staff ?? res?.employees ?? []))
      .catch(() => {})
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await employeeService.update(selectedPharmacy.id, editingId, payload);
        toast.success(t("employees.employeeUpdated"));
      } else {
        const newEmployee = await employeeService.create(selectedPharmacy.id, payload);
  ...
  const handleDelete = async (emp) => {
    ...
    try {
      await employeeService.remove(selectedPharmacy.id, emp.id);
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-surface border-b border-surface-container-high">
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("employees.name")}</th>
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("employees.email")}</th>
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("employees.phone")}</th>
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("employees.role")}</th>
                    <th className="text-left px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("employees.permissions")}</th>
                    <th className="text-right px-6 py-4 text-xs uppercase tracking-widest text-on-surface-variant font-bold">{t("app.actions")}</th>
                  </tr>
                </thead>
