...
export default function PendingDoctors() {
  ...
  const fetchPending = async () => {
    setLoading(true);
    try {
      const res = await adminApi.doctors.pendingVerifications();
      setDoctors(res.data || []);
    } catch {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await adminApi.doctors.verify(actionTarget.id, payload);
      toast.success(
        actionType === "approve" ? "Doctor approved successfully" : "Doctor rejected"
      );
      ...
      toast.error(
        actionType === "approve" ? "Failed to approve doctor" : "Failed to reject doctor"
      );
    } finally {
      setProcessing(false);
    }
    ...
    <div>
      ...
      {doctors.length === 0 ? (
        <EmptyState
          ...
          description="All doctors have been reviewed."
        />
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 mb-4 text-sm">
                  ...
                  <p className="text-on-surface-variant">
                    <span className="font-medium text-on-surface">Specialization:</span>{" "}
                ...
                {doctor.syndicate_card_image && (
                  <div className="mb-4">
                    ...
                    <img
                      ...
                      alt="Syndicate Card"
                      className="w-full rounded-xl border border-surface-container-high"
                    />
                  </div>
                )}
      ...
      {actionType === "reject" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-surface-container-lowest rounded-2xl shadow-2xl border border-surface-container-high max-w-md w-full p-6 animate-[fadeIn_0.2s_ease]">
            <h3 className="text-lg font-bold text-on-surface mb-2">Reject Doctor</h3>
