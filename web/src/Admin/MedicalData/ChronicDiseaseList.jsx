...
export default function ChronicDiseaseList() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminApi.chronicDiseases.list(params);
      if (page === 1 || !isForward) {
        setData(res.data || []);
      } else {
      ...
      toast.error(err?.response?.data?.message || "Failed to load chronic diseases");
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await adminApi.chronicDiseases.delete(deleteTarget.id);
      toast.success("Chronic disease deleted successfully");
      setDeleteTarget(null);
      fetchList();
    } catch (err) {
      toast.error(err?.response?.data?.message || "Failed to delete chronic disease");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Code",
      render: (row) => (
        <p className="text-sm text-on-surface-variant font-mono">{row.code || "—"}</p>
      ),
    },
    {
      key: "category",
      header: "Category",
      render: (row) => (
        <p className="text-sm text-on-surface truncate">{row.category || "—"}</p>
      ),
    },
    {
      key: "created_at",
    ...
    },
    {
      ...
      render: (row) => (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/Admin/MedicalData/Diseases/${row.id}/edit`); }}
        ...
        <div className="relative">
          ...
          <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-on-surface">Chronic Diseases</h1>
                <p className="text-on-surface-variant mt-1">Manage chronic disease records</p>
              </div>
            </div>
            <PageLoading />
          </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-surface">
      <div className="relative">
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              onClick={() => navigate("/Admin/MedicalData/Diseases/create")}
          ...
          <DataTable
            ...
            emptyTitle="No chronic diseases found"
            emptyDescription="Try adjusting your search or add a new disease."
          />
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        message={deleteTarget ? `Are you sure you want to delete ${deleteTarget.name_en || deleteTarget.name_ar}? This action cannot be undone.` : ""}
        confirmLabel="Delete"
        cancelLabel="Cancel"
