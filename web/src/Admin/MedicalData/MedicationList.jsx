...
export default function MedicationList() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminApi.medications.list(params);
      if (page === 1 || !isForward) {
        setData(res.data || []);
      } else {
      ...
      toast.error(err?.response?.data?.message || "Failed to load medications");
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await adminApi.medications.delete(deleteTarget.id);
  ...
  const columns = [
    {
      ...
      header: "Trade Name",
      render: (row) => (
        <p className="text-sm font-bold text-on-surface truncate">{row.trade_name || "—"}</p>
      ),
    },
    {
      key: "form",
    ...
    },
    {
      ...
      header: "Arabic Form",
      render: (row) => (
        <p className="text-sm text-on-surface truncate">{row.arabic_form || "—"}</p>
      ),
    },
    {
      key: "manufacturer",
      header: "Manufacturer",
      render: (row) => (
        <p className="text-sm text-on-surface-variant truncate">{row.manufacture?.name || "—"}</p>
      ),
    },
    {
      key: "barcode",
      header: "Barcode",
      render: (row) => (
        <p className="text-xs text-on-surface-variant font-mono">{row.barcode || "—"}</p>
      ),
    },
    {
      key: "created_at",
    ...
    },
    {
      ...
      render: (row) => (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/Admin/MedicalData/Medications/${row.id}/edit`); }}
        ...
        <div className="relative">
          ...
          <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-on-surface">Medications</h1>
                <p className="text-on-surface-variant mt-1">Manage medication records</p>
              </div>
            </div>
            <PageLoading />
          </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-surface">
      <div className="relative">
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              onClick={() => navigate("/Admin/MedicalData/Medications/create")}
          ...
          <DataTable
            ...
            emptyDescription="Try adjusting your search or add a new medication."
          />
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        message={deleteTarget ? `Are you sure you want to delete ${deleteTarget.trade_name}? This action cannot be undone.` : ""}
        confirmLabel="Delete"
        cancelLabel="Cancel"
