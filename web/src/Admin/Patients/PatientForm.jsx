...
export default function PatientForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await adminApi.patients.show(id);
        const p = res.data || res;
        setForm({ user_id: p.user_id || "" });
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await adminApi.patients.update(id, form);
        toast.success("Patient updated successfully");
      } else {
        await adminApi.patients.create(form);
      ...
      <div className="h-full overflow-y-auto bg-surface">
        <div className="relative">
          ...
          <div className="relative max-w-7xl mx-auto px-8 py-10 pb-32">
            <PageHeader icon="person" title={isEdit ? "Edit Patient" : "Add Patient"} />
            <PageLoading />
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <div className="relative max-w-3xl mx-auto px-8 py-10 pb-32">
          <PageHeader
            ...
            description={isEdit ? "Update patient information" : "Register a new patient"}
          />
