...
export default function ScientificRepList() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const res = await adminApi.scientificReps.list(params);
      if (page === 1 || !isForward) {
        setData(res.data || []);
      } else {
      ...
      toast.error("Failed to load scientific reps");
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await adminApi.scientificReps.delete(deleteTarget.id);
      toast.success("Scientific rep deleted successfully");
      setDeleteTarget(null);
      fetchList();
    } catch {
      toast.error("Failed to delete scientific rep");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Company",
      render: (row) => (
        <p className="text-sm text-on-surface truncate">{row.company?.commercial_name || "—"}</p>
      ),
    },
    {
      key: "created_at",
    ...
    },
    {
      ...
      render: (row) => (
        <>
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/Admin/ScientificReps/${row.id}`); }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); navigate(`/Admin/ScientificReps/${row.id}/edit`); }}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-on-surface-variant hover:bg-primary/[0.08] hover:text-primary transition-all"
            title="Edit"
          >
        ...
        <div className="relative">
          ...
          <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-on-surface">Scientific Reps</h1>
                <p className="text-on-surface-variant mt-1">Manage all registered scientific representatives</p>
              </div>
            </div>
            <PageLoading />
          </div>
      ...
      <div className="relative">
        ...
        <div className="p-6 lg:p-10 max-w-7xl mx-auto flex flex-col gap-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              onClick={() => navigate("/Admin/ScientificReps/create")}
          ...
          <DataTable
            ...
            emptyIcon="biotech"
            emptyTitle="No scientific reps found"
            emptyDescription="Try adjusting your search or add a new scientific rep."
          />
        </div>
      </div>
