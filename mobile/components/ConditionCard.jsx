...
export default function ConditionCard({ name, sub, icon, selected, onPress, showIcon = true }) {
  ...
  return (
    ...
    >
      ...
      <View>
        <Text style={[tw`font-bold leading-tight`, { fontSize: fontScale(16), color: selected ? theme.primary : theme.onSurface }]} numberOfLines={1}>
          {name}
        </Text>
        {sub ? <Text style={[{ fontSize: fontScale(10), opacity: 0.6 }, { color: theme.onSurfaceVariant }]} numberOfLines={1}>{sub}</Text> : null}
      </View>
    </TouchableOpacity>
  );
}
